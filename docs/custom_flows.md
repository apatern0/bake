...
The `flow_options` config attribute on each step passes a dictionary of options to the flow
script at runtime. The flow receives them as a JSON string in a template variable or may read them
from the *bake* config object if the flow is implemented in Python and imports `bake.context`:

```python
# In manifest — pass extra options to the custom flow
config.vrf.flow_options = {
    "icarus": ["-Wall"],
    "xcelium": ["-64bit"],
}
```

The built-in `run.py` template uses `flow_options` keyed by simulator name. Custom flow scripts
can access the same data by reading `$BAKE_SIM_OPTIONS` in the template (which is populated from
the simulator-specific entry in `vrf_options` declared in the test, not `flow_options`).
