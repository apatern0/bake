...
# The flow runs under the system python3, which need not have bake's
# dependencies; plain logging is good enough when coloredlogs is missing.
try:
    import coloredlogs
except ImportError:
    coloredlogs = None
...
if __name__ == "__main__":
    if coloredlogs is not None:
        ...
        logging.basicConfig(level=logging.INFO, format="[tmrg ] %(levelname)s\t %(message)s")
    try:
        main()
        sys.exit(0)
