...
- **`manifest`** — a Python file (always named literally `manifest`, no extension) that is `exec`'d by bake. It calls `block()`, `test()`, `env()`, `lib()`, `flow()`, and `add_steps_dir()` to register design objects into the global `context`.
...
- **`StepData`** — a dataclass threaded through the pipeline. Initialized from `BlockSpec`/`EnvSpec`, then freely mutated by each step. `target` and `test` fields are **read-only** (immutable originals); all working state (`top`, `rtl_files`, `vrf_files`, etc.) is mutable. Steps **must not** access `context` registries at runtime — only `self.data` and `self.config`.
...
4. `Recipe.elaborate()` creates a `StepData` from the block/test, instantiates each `Step` in chain order, and threads `output_data` forward.
...
Subclass `Step`, set `name`, implement `run(self)`, `source_files`, and `output_files`. Register the step directory in a manifest with `add_steps_dir("path/to/steps")`. The class is auto-discovered from any `.py` file in that directory defining a `Step` subclass.
...
| `WARNING` | Recoverable situations the user should know about: skipped includes due to `skip_on_include_errors`, all-symlink output files, missing optional resources. |
...
- `BakeIncludeError` — missing or stale block/env include; may be suppressed by `skip_on_include_errors`.
