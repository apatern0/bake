...
- Add or update a test for behaviour changes. `test/test_bake.py` drives the CLI in-process on
  the projects under `test/projects/` — real manifests you can `cd` into and run by hand; the
  `project` fixture copies them to a temporary directory first. Add a project directory for a
  new scenario rather than writing manifest text inside a test.
